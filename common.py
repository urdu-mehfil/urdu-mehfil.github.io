#!/usr/bin/env python3
"""
common.py — shared logic used by extract.py, discover.py, and
indexes.py: HTTP fetching, HTML cleaning, page templates, nav
building, and filename-sanitizing helpers. Not run directly.

Retry strategy (deliberately split two ways):
  - Transport-level failures (connection resets, timeouts, 500/502/504)
    are retried automatically by the session's mounted HTTPAdapter +
    urllib3.util.Retry, with exponential backoff. No code needed here
    for that case — it's handled transparently below the level of get().
  - 429/503 are handled manually, because those need to pause EVERY
    worker thread sharing this session, not just retry the one request
    that got throttled. urllib3's Retry has no concept of that kind of
    cross-thread coordination, so it stays hand-rolled.
"""
...
# Shared across all worker threads (both the outer per-thread pool and
# each thread's own inner per-page pool, if nested concurrency is in
# use — this is a plain module-level global, so it coordinates across
# every level of nesting automatically): if the server ever returns
# 429/503 to ANY request, every worker pauses until this timestamp
...
def parse_total_pages(html: str) -> int:
    """Reads a thread's total page count off its own page-1 pagination
    nav (a link with title="Last", and/or literal "X از Y" text). This
    is what lets extract.py learn the page count from the SAME fetch
    it needed for content anyway, instead of a separate discovery-only
    request just to learn the number."""
...
def get(url, retries=RETRY_ATTEMPTS):
    """5xx/connection-level retries are handled transparently by the
    session's mounted HTTPAdapter+Retry — no manual sleep loop needed
    for those. This function's own retry loop exists specifically for
    429/503, which need the shared cross-thread backoff below."""
    last_exc = None
    for attempt in range(1, retries + 1):
        ...
        try:
            ...
            # HTTPAdapter's Retry already exhausted ITS attempts for
            # transport/5xx issues by the time we see this exception.
            last_exc = e
            logger.error(f"Request failed after transport-level retries: {url} ({e})")
            continue
...
def get_many(urls, max_workers=1):
    ...
    are simply omitted rather than raising, so one bad page doesn't
    take down a whole batch.

    Default is 1 (sequential). If nesting this inside an outer
    per-thread pool too, remember: total concurrent requests = outer
    workers x this max_workers. Size both deliberately.
    """
    results = {}
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for future in as_completed(future_to_url):
            ...
            try:
                ...
                logger.error(f"Failed permanently: {u} ({e})")
    return results
...
def safe_dirname(title: str, thread_id, max_bytes: int = 100) -> str:
...
def sanitize_category_dir_name(category_name: str) -> str:
    ...
    directory name — kept in one place so extract.py and indexes.py
